  ...
  return (
    <>
      <motion.div
        ...
        className="shadow-md p-4 md:w-1/2 rounded-lg space-y-5">
        <div className="flex gap-2">
          <h2 className="text-xl font-semibold">{item?.name}</h2>
          {
            discount > 0 && (
              <Badge
                className="bg-gradient-to-r from-primary via-purple-50-500 to-pink-500 border-pink-500 h-6"
              >
                -{discount}%
              </Badge>
            )
        ...
        <div className="leading-5 text-muted-foreground">
          ...
          <p>Kho hàng: {stock > 0 && "Còn hàng" || "Tạm hết hàng"}</p>
        </div>
