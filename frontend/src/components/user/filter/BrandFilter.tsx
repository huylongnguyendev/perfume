...
const BrandFilter = ({ filters }: Props) => {
  ...
  const handleSelectBrand = (brandName: string) => {
    dispatch(fetchAllProduct({ ...filters, brand: brandName }))
      ...
      <div>
        ...
        <ul className="w-full rounded-lg overflow-y-scroll h-40 border bg-background font-semibold p-1">
          <li
            ...
            className={cn("px-4 py-1 rounded-md transition-colors hover:text-primary hover:bg-muted cursor-pointer", brandSelected === "" && "bg-primary/10 text-primary")}>
            Tất cả
          </li>
          {
