        ...
        <>
            <div className={cn("shadow-md rounded-md p-2")}>
                <div className="relative group overflow-hidden flex justify-center items-center">
                    <img src={prd} alt={product._id} width={144} height={144} className="object-cover transition-all duration-300 group-hover:scale-110" />
                    {
                        product.volumes.map(vol => vol.sku === volSelect && vol.discount > 0 && (
                            <Badge key={vol.sku} variant="destructive" className="absolute top-4 left-4">-{vol.discount}%</Badge>
                        ))
                    }
